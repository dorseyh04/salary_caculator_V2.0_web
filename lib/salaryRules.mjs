...
export function parseSettlementRows(rows) {
  ...
  const col = {
    ...
    negativeFlag: findHeaderIndex(headerRow, (header) => header.includes("负毛利")),
    interimFee: findHeaderIndex(headerRow, (header) => header.includes("居间费") || header.includes("居间单价")),
  };
  ...
  for (let i = headerIndex + 1; i < rows.length; i += 1) {
    ...
    const classificationRaw = toText(getByIndex(row, categoryCol, 19));
    // 行级回退：分类列为空时，尝试读「类别」列（兼容新旧模板）
    const classification = classificationRaw || (legacyCategoryCol >= 0 && legacyCategoryCol !== categoryCol ? toText(row[legacyCategoryCol]) : "");
    settlements.push({
      _rowIdx: i + 1,
      ...
      负毛利标记: toText(getByIndex(row, col.negativeFlag, 25)),
      居间费: toMoneyNumber(getByIndex(row, col.interimFee, 26), 0),
    });
  }
